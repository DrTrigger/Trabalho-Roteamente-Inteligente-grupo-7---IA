...
support_team = Team(
    ...
    db=db,
    instructions=[
        "Você é o roteador inteligente do IFES.",
        "Analise a solicitação do aluno e delegue para o setor correto:",
        "- TI: problemas técnicos, senhas, sistemas, acesso, e-mail, rede.",
        "- Secretaria: matrícula, documentos, trancamento, histórico escolar, requerimentos.",
...
WELCOME_MSG = Message(
    ...
    content=(
        ...
        "- 📋 **Secretaria** — matrícula, documentos, trancamentos\n\n"
...
class State(rx.State):
    ...
    def on_load(self):
        ...
        # Restaura histórico salvo no navegador
    ...
    def _save_history(self):
        """Salva o histórico visual no LocalStorage"""
        try:
            serializable = [
                {
                    "role": m.role,
            ...
            self.chat_history_json = json.dumps(serializable)
        except Exception:
            pass
    ...
    async def handle_submit(self):
        ...
        # ==================== DETECÇÃO DE PERGUNTAS SOBRE HISTÓRICO ====================
        meta_keywords = [
            "última dúvida", "ultima duvida", "o que eu perguntei",
            "qual foi minha", "minhas perguntas", "histórico da conversa",
            "o que eu falei", "última mensagem", "contexto anterior"
        ]
        is_meta = any(kw in user_message.lower() for kw in meta_keywords)

        if is_meta:
            # Responde direto do histórico local (rápido e confiável)
            ultimas_user = [m for m in self.chat_history if m.role == "user"][-5:]
            if ultimas_user:
                conteudo = "**Aqui estão suas últimas interações:**\n\n"
                for i, m in enumerate(ultimas_user, 1):
                    conteudo += f"{i}. {m.content}\n"
            else:
                conteudo = "Ainda não temos interações anteriores nesta conversa."
        ...
        # Chama o Team (Agno) para perguntas normais
        try:
            response = support_team.run(
                user_message,
                session_id=self.session_id,
            )
            bot_reply = response.content if response and response.content else "Não foi possível obter uma resposta no momento."

            # Define o badge do agente
            lower_reply = bot_reply.lower()
            if any(k in lower_reply for k in ["senha", "sistema", "acesso", "vpn", "e-mail", "login", "rede", "infra"]):
                agent_name = "🖥️ Equipe de TI"
            elif any(k in lower_reply for k in ["secretaria", "matrícula", "documento", "trancamento", "histórico", "requerimento"]):
                agent_name = "📋 Secretaria Acadêmica"
            else:
        ...
        self.chat_history.append(
            Message(role="bot", content=bot_reply, agent=agent_name, timestamp=now)
        )
        self._save_history()
        self.is_processing = False
    ...
    return rx.box(
        rx.hstack(
            ...
            rx.button(
                ...
                border="1px solid rgba(255,255,255,0.25)", border_radius="8px",
...
def sector_chips() -> rx.Component:
    ...
    return rx.hstack(
        ...
        padding_x="20px", padding_y="10px", bg="#fafbff", border_bottom="1px solid #e8eaf6", class_name="no-print",
    )
    ...
    return rx.box(
        rx.hstack(
            rx.input(
                ...
                width="100%", disabled=State.is_processing, size="3",
                border_radius="12px", border="1.5px solid #c5cae9",
                padding_x="16px", bg="white", color="#263238",
            ...
            rx.button(
                ...
                color="white", border_radius="12px", padding_x="22px", padding_y="0",
                height="44px", disabled=State.is_processing, cursor=rx.cond(State.is_processing, "not-allowed", "pointer"),
        ...
        rx.hstack(
            ...
            rx.text("Clique em Enviar • A IA identifica e encaminha para TI ou Secretaria automaticamente", size="1", color="#90a4ae"),
...
def footer_info() -> rx.Component:
    return rx.hstack(
        ...
        padding_x="20px", padding_y="8px", bg="#f4f6fb", border_top="1px solid #e8eaf6", width="100%", class_name="no-print",
    )
...
@media print {
    ...
    .portal-header { background: #1a237e !important; print-color-adjust: exact; -webkit-print-color-adjust: exact; padding: 12px 20px !important; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (el) {
        ...
        rx.vstack(
            ...
            input_area(),
            footer_info(),
            spacing="0", width="100%", height="100%",
            border_radius="16px", overflow="hidden",
            box_shadow="0 8px 40px rgba(26,35,126,0.18)",
        ),
